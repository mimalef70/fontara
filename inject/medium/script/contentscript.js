...
browser.storage.local.get(['font', 'medium'], function(items) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'medium', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.medium != undefined || changes.font != undefined) {
      if (items.medium == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
