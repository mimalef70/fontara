...
const FontUploader = () => {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (!validTypes.includes(extension)) {
                alert("لطفا یک فایل فونت معتبر (ttf, woff, woff2, otf) انتخاب کنید")
                return
            }

            if (file.size > 2 * 1024 * 1024) {
                alert("حجم فایل فونت نباید بیشتر از 2 مگابایت باشد")
                return
            }
    ...
    const handleSaveFont = async () => {
        if (!selectedFile || !fontName) {
            alert("لطفا فایل فونت و نام فونت را وارد کنید")
            return
        }
        ...
        try {
            ...
            alert(error instanceof Error ? error.message : "خطا در پردازش فونت")
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleDeleteFont = async (fontName: string) => {
        try {
            ...
            alert("خطا در حذف فونت")
        }
    }
    return (
        <div className="p-4 max-w-xl mx-auto">
            <div className="bg-white rounded shadow p-6">
                <h2 className="text-xl font-bold text-right mb-4">افزودن فونتsdsdsd دلخواه</h2>
