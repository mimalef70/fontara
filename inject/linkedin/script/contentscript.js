...
browser.storage.local.get(['font', 'linkedin'], function(items) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'linkedin', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.linkedin != undefined || changes.font != undefined) {
      if (items.linkedin == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
