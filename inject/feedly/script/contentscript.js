...
browser.storage.local.get(['font', 'feedly'], function(items) {
  if (items.feedly == true || items.feedly == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'feedly', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.feedly != undefined || changes.font != undefined) {
      if (items.feedly == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
