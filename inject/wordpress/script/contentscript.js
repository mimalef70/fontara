...
browser.storage.local.get(['font', 'wordpress'], function(items) {
  if (items.wordpress == true || items.wordpress == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'wordpress', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.wordpress != undefined || changes.font != undefined) {
      if (items.wordpress == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
