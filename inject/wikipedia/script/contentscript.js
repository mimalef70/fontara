...
browser.storage.local.get(['font', 'wikipedia'], function(items) {
  if (items.wikipedia == true || items.wikipedia == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'wikipedia', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.wikipedia != undefined || changes.font != undefined) {
      if (items.wikipedia == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
