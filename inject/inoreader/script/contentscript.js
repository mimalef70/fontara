...
browser.storage.local.get(['font', 'inoreader'], function(items) {
  if (items.inoreader == true || items.inoreader == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'inoreader', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.inoreader != undefined || changes.font != undefined) {
      if (items.inoreader == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
