...
browser.storage.local.get(['font', 'trello'], function(items) {
  if (items.trello == true || items.trello == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'trello', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.trello != undefined || changes.font != undefined) {
      if (items.trello == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
