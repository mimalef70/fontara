...
browser.storage.local.get(['font', 'virgool'], function(items) {
  if (items.virgool == true || items.virgool == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'virgool', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.virgool != undefined || changes.font != undefined) {
      if (items.virgool == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
