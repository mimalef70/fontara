...
browser.storage.local.get(['font', 'google'], function(items) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'google', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.google != undefined || changes.font != undefined) {
      if (items.google == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
