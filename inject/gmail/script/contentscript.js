...
browser.storage.local.get(['font', 'gmail'], function(items) {
  if (items.gmail == true || items.gmail == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'gmail', 'custom_fonts'], function(items) {
    ...
    if (changes.gmail != undefined || changes.font != undefined) {
      if (items.gmail == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
