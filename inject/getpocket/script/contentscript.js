...
browser.storage.local.get(['font', 'getpocket'], function(items) {
  if (items.getpocket == true || items.getpocket == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'getpocket', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.getpocket != undefined || changes.font != undefined) {
      if (items.getpocket == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
