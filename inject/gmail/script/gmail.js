...
var obsRun = false;
browser.storage.local.get('gmail', function(items) {
  if (items.gmail == true || items.gmail == undefined) {
    ...
    let run_on_page = () => {
      let post_articles = document.querySelectorAll(
        'p,h1,h2,h3,h4,h5,h6,span,div:not(.gmail_default):not(.editable):not([class*="gmail"]),a,strong'
      );
