...
| Unit tests | شکست: 47 پاس، 2 fail |
| Biome lint | شکست فرمت در یک فایل تست |
| Git worktree | قبل از افزودن این گزارش تمیز بود؛ اکنون همین فایل گزارش untracked است |

جمع بندی ریسک: پروژه از نظر معماری اصلی بالغ است و برای extension runtime طراحی های خوبی دارد، مثل پردازش chunked DOM، MutationObserver coalescing، و sanitization برای CSS/font data. اما CI فعلی به خاطر تست و lint fail می شود. همچنین permission های بسیار گسترده، custom CSS های `body *`، هشدارهای Firefox برای `innerHTML` در bundle، و نگهداری فونت سفارشی به صورت base64 در storage مهم ترین ریسک ها هستند.
...
| `assets/styles` | CSS اختصاصی ChatGPT, Gemini, LinkedIn, WhatsApp, X |
| `tasks` | build pipeline دستی با esbuild/PostCSS/yazl |
| `tests/unit` | 49 تست واحد، فعلا 2 fail |
...
21 سایت popular در `src/config/sites.ts` تعریف شده اند. همه در default فعال هستند. پنج سایت مسیر custom CSS دارند:
...
نکته: `build:all` خروجی های platform را تازه می سازد، اما کل پوشه `build` را پاک نمی کند. در بررسی فعلی یک source package قدیمی `firefox-mv3-source-4.2.0` در build باقی مانده بود، در حالی که نسخه package فعلی 4.2.1 است. چون `build/` ignored است، ریسک اصلی آن سردرگمی محلی است نه commit.
...
|---|---|---|---|
| بالا | Unit tests fail | دو تست CSS برای ChatGPT و LinkedIn fail هستند، چون CSS فعلی variable/rule های مورد انتظار را ندارد | یا CSS را مطابق تست کامل کنید، یا تست ها را با refactor جدید هماهنگ کنید |
| بالا | CI/release فعلا می شکند | `pnpm test` و `pnpm lint` fail هستند، workflow هم همین ها را اجرا می کند | قبل از release اصلاح شود |
| متوسط | Biome lint fail | فقط formatting در `tests/unit/inject-performance.test.ts` | اجرای `pnpm lint:fix` یا اصلاح دستی |
...
| متوسط | custom CSS با `body *` | در ChatGPT/LinkedIn ممکن است code/pre/icon ها یا UI غیرمتنی را خراب کند | مثل X برای monospace/code exception بگذارید |
| متوسط | README می گوید unlimited custom fonts | کد محدودیت 2MB per file و محدودیت storage دارد | متن README را دقیق کنید |
| متوسط | migration محدود به versioned sites | سایت های بدون `version` در default list اگر بعدا regex/icon آنها عوض شود migrate نمی شوند | برای همه سایت های default version بگذارید یا migration rule عمومی تر بسازید |
...
| پایین | stale build artifacts | source package 4.2.0 در build مانده بود | قبل از package source یا release، build root را پاک کنید یا script cleanup بسازید |
...
|---|---|
| `pnpm typecheck` | پاس |
| `pnpm test` | fail، 49 تست، 47 pass، 2 fail |
| `pnpm lint` | fail، یک خطای formatter |
...
1. `ChatGPT uses site CSS instead of streaming DOM writes`: در `assets/styles/chatgpt.css` متغیر `--fontara-chatgpt-monospace` وجود ندارد.
2. `LinkedIn uses site CSS instead of streaming DOM writes`: در `assets/styles/linkedin.css` متغیر `--fontara-linkedin-monospace` وجود ندارد و تست همچنین `--artdeco-reset-typography-font-family-sans` را انتظار دارد.
...
1. تست و lint را اول درست کنید تا CI سبز شود.
2. ChatGPT/LinkedIn CSS را با exception های monospace/code و متغیرهای fallback کامل کنید.
3. README را با واقعیت custom font limits و فعال سازی سایت سفارشی هماهنگ کنید.
4. برای permission ها یک تصمیم محصولی بگیرید: اگر همه URL ها لازم است، در مستندات reviewer توضیح دهید؛ اگر نه، optional permissions را بررسی کنید.
5. برای options page نمایش مصرف storage و تعداد/حجم فونت ها اضافه کنید.
6. برای همه سایت های default `version` بگذارید تا migration آینده قابل پیش بینی باشد.
7. هشدار Firefox `innerHTML` را تا منبع دقیق در bundle trace کنید و اگر ناشی از React/dependency است در source package review note توضیح دهید.

## جمع بندی

FontARA V4 از نظر ایده و معماری یک افزونه فونت چنجر جدی و نسبتا خوب مهندسی شده است. مسیرهای runtime برای حالت generic، سایت های custom CSS، فونت سفارشی، editor ها و تغییرات زنده DOM طراحی شده اند. مشکل اصلی فعلی نه در build، بلکه در کیفیت انتشار است: تست ها و lint fail هستند، و چند ریسک permission/CSS/reviewer باید قبل از release حل یا مستند شوند.
