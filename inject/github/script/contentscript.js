...
browser.storage.local.get(['font', 'github'], function(items) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'github', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.github != undefined || changes.font != undefined) {
      if (items.github == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
