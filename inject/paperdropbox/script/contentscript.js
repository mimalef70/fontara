...
browser.storage.local.get(['font', 'paperdropbox'], function(items) {
  if (items.paperdropbox == true || items.paperdropbox == undefined) {
...
browser.storage.onChanged.addListener(function(changes, namespace) {
  browser.storage.local.get(['font', 'paperdropbox', 'custom_fonts'], function(
    items
  ) {
    ...
    if (changes.paperdropbox != undefined || changes.font != undefined) {
      if (items.paperdropbox == true) {
        document.documentElement.style.setProperty(`--font`, items.font);
      } else {
        document.documentElement.style.setProperty(`--font`, 'remove font');
      }
